...
import { getUserProfile, updateProfile, getUserAddresses, getUserOrderHistory } from '@/services/users.service'
...
export async function PUT(request: NextRequest) {
  ...
  if (pathSegments.length === 4 && pathSegments[3] === 'addresses') {
    ...
    return NextResponse.json(
      { success: false, error: 'Use /api/users/addresses/[id] for updating addresses' },
      { status: 400 }
    )
  } else {
