...
import { registerUser, loginUser, getCurrentUser, requestPasswordReset, changePassword } from '@/services/auth.service'
...
export async function POST(request: NextRequest) {
  ...
  if (pathSegments.length === 3 && pathSegments[2] === 'register') {
    ...
    return NextResponse.json(result)
  } else if (pathSegments.length === 3 && pathSegments[2] === 'reset-password') {
    ...
    return NextResponse.json(result)
  } else if (pathSegments.length === 3 && pathSegments[2] === 'change-password') {
